<script lang="ts">
    ...
    import Dialogs from "$lib/components/UI/Dialogs.svelte" 
    import TillSticker from "$lib/assets/images/github.svg" 
    import "./styles.css"
	...
	<nav class="px-5 py-2 text-base shadow flex items-center max-sm:text-sm">
        ...
        <div class="flex items-center justify-center gap-2 ml-auto">
		    <div class="sm:flex sm:gap-2 sm:items-center none">
                <a href="https://github.com/imrany" target="_blank" rel="noreferrer noopener" class="flex items-center justify-center gap-1">
                    <!-- <span class="material-symbols-outlined text-base">open_in_new</span>  -->
                    <img src="{TillSticker}" width="21" height="21" alt="Github logo"/>
                    <span>Github</span>
                </a>
		    </div>
            <button class="px-2 py-1 text-white rounded-md border-slate-600 bg-slate-500 flex gap-1 items-center justify-center" on:click={()=>openDialog("open_support_dialog")}>
